(* ========================================================================== *)
(*  08_response_matrix  (1 eV threshold)
*)
(*  Choose the number of neutrino-energy intervals, integrate the response functions of 06_response_functions (.wdx) over each interval and export the response matrices as csv to ../output. From 1_Main.nb.
*)
(* ========================================================================== *)
...
For[i = 1, i <= Length[CRlistbox], i++,
  edges = makeEvEdgesByCount[0.18*MeV, 2*MeV, n, 20, 2*MeV];
...
t4R = 0;
For[i = 1, i <= Length[CRlistbox], i++,
  edges = makeEvEdgesByCount[0.18*MeV, 7*MeV, n, 20, 2*MeV];
  t4R = SampleIntByEdges[CRlistbox[[i]]][edges];
  AppendTo[CRmat, t4R]];
...
For[i = 1, i <= Length[CRlist], i++,
  t4R = SampleInt[CRlist[[i]]][0.18*MeV, (6.82/n)*MeV, n];
  AppendTo[CRmat, t4R]];
